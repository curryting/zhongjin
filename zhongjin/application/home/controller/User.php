...
class User extends Controller
{
    ...
    public function login()
    {
		...
		session('user',$id);
		$memInfo = UserLogic::getNameByUserids([$id]);
		return ['code'=>Errcode::SUCCESS, 'username'=>$memInfo[$id]];
    }
	...
	public function pwdFind()
	{
		...
		$code = UserLogic::pwdFind($username, $pwd, $phoneCode);
		switch($code){
			case Errcode::OLD_NEW_EQUALLY:
				$msg = '新密码和旧密码一致';
				break;
			case Errcode::PHONR_CODE_ERR:
				$msg = '手机验证码错误';
				break;
	...
	{
		$username = input('param.username', '', 'trim');
		empty($username) && $this->result('', Errcode::PARAM_ERR, '请输入正确的账号');
	...
	public function add()
	{
		...
		(empty($account) || empty($pwd) || !is_mobile($mobile) || empty($name))
			&& $this->result('', Errcode::PARAM_ERR, '参数错误');
			
		$res = UserLogic::add($account, $pwd, $mobile, $name);
		
		$res === FALSE && $this->result('', Errcode::INSERT_ERR, '添加管理员失败');
		$res == -1 && $this->result('', Errcode::USER_ISSET, '管理员已存在');
		
		try{
			WxLogic::admin_sync_qy([$res], 'sync');
		}catch(\Exception $e){
