...
/** 获取API授权TOKEN */
function api_token($bflush=false, $appkey='')
{
    static $_key = '';
    static $_token = '';
    if($bflush) $_token = '';
    
    empty($appkey) || $_key = $appkey;
    if(empty($_key))    return '';
    
    $config = config($_key);
    if(empty($config))  return '';
    
    $_token = empty($_token) ? ($bflush ? '' : Cache($_key)) : $_token;
    if(empty($_token)) {
        $api = APP_HTTP.API_WEB_DOMAIN.'/auth/get_token';
    	$param['appkey'] = $config['key'];
    	$param['secret'] = $config['secret'];
        
        $ret = curl_request_json($error, $api, $param, 'GET');
        $_token = isset($ret['token']) ? $ret['token'] : '';
        empty($_token) || Cache($_key, $_token, $ret['expire']);
    }
    
    return $_token;
}

function api_http_get($url, $param)
{
    $err = -1;
    $param['token'] = api_token();
    $response = curl_request_json($err, $url, $param, 'GET');
    if(isset($response['errcode']) && intval($response['errcode']) === 10001) {
        $param['token'] = api_token(true);
        $response = curl_request_json($err, $url, $param, 'GET');
    }
    
    if(is_array($err) && !empty($err)) {
        $response['errcode'] = $err[0];
        $response['errmsg'] = $err[1];
    }
    
    return $response;
}

function api_http_post($url, $param, $timeout=20)
{
    $pos = stripos($url, '?');
    $token = ((false === $pos) ? '?token=' : '&token=').api_token();
    
    $err = -1;
    $param = json_encode($param);
    $header_ex = ['Content-Type: application/json', 'Content-Length: '.strlen($param)];
    
    $response = curl_request_json($err, $url.$token, $param, 'POST', $timeout, $header_ex);
    if(isset($response['errcode']) && intval($response['errcode']) === 10001) {
        $token = ((false === $pos) ? '?token=' : '&token=').api_token(true);
        $response = curl_request_json($err, $url.$token, $param, 'POST', $timeout, $header_ex);
    }
...
include(APP_PATH .'common/common/wxapi/weixin.php');

/** 获取微信jssdk 参数*/
function qy_get_jsapi_sign($qid=0, $agentid=-1, $funcname='')
{
    $jsapi = qy_get_jsapi($qid, $agentid, $funcname);
    if(false === $jsapi) return [];

    //$url = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
	//$url = 'http://im.weiwojiaoyu.com/public/weixin/im/index.html?qid/20/aid/1000009';
	$url = APP_HTTP.$_SERVER['HTTP_HOST'].'/public/weixin/im/index.html?qid/'.$qid.'/aid/'.$agentid;
    $timestamp = time();
    $nonceStr = get_random_str(16);

    //这里参数的顺序要按照 key 值 ASCII 码升序排序
    $raw_string = "jsapi_ticket={$jsapi['ticket']}&noncestr=$nonceStr&timestamp=$timestamp&url=$url";
 ...
 * @return false or 文件上传后的url
*/
function oss_sdk_upload($filename, $md5_data)
{
    vendor('oss_php_sdk.sdk', '.class.php');

    $spos = strrpos($filename, '/');
    $epos = strrpos($filename, '.');
    $spos = ($spos === false) ? 0 : ($spos + 1);
    $epos = ($epos === false) ? (strlen($filename) - $spos) : $epos;
    $fname_ex = substr($filename, $spos);
    $fname = substr($filename, $spos, intval($epos - $spos));
    $obj_d = empty($md5_data) ? md5_file($filename) : $md5_data;
    $obj_d = str_split($obj_d, intval(strlen($obj_d) / 4));
    $obj_d = implode('/', $obj_d);
    ...
    try
    {
        $response = $oss_sdk_service->create_object_dir($bucket, $obj_d);
        $response = $oss_sdk_service->upload_file_by_file($bucket, $obj_d . '/' . $fname_ex, $filename);
        if(intval($response->status) == 200){
            return "http://$bucket.oss.aliyuncs.com/$obj_d/$fname_ex";
        }
	}catch (\Exception $ex){
        trace('oss_sdk_upload Exception: ' . $ex->getMessage(), 'OSS');
    }
...
/** 获取随机数字 */
function get_random_code($random_key = '', $length = 0)
{
    srand((double)microtime()*1000000);
    $random_number = rand(100000, 999999) . '';
    $random_string = get_random_str(2);

    $random_code = md5($random_string . md5($random_number));

    if(!empty($random_key)) $random_code = md5($random_code . md5($random_key));

    if($length > 0)
    {
        $code = crc32($random_code);
        $code = strval($code);
...
{
	$pattern = '/^1[3-5,7-9]{1}[0-9]{9}$/';
	$rtn_match = preg_match($pattern, $mobile);
	if($rtn_match == 1){
		return true;
	}else{
		return false;
...
/** 字符串切分 */
function str_split_unicode($str, &$len, $l = 0)
{
	if ($l > 0){
		$ret = [];
...
/** 生成企业号中的userid */
function create_userid_by_name_mobile($name, $mobile)
{
    // 参数不正确返回随机串
    if(empty($name) || !is_mobile($mobile))   return get_random_str(12);
    
    $mobile = md5($mobile);
    if(preg_match('/^[\x{4e00}-\x{9fa5}]+$/u', $name) > 0){// 全是中文
        $new_name = '';
        $name_len = mb_strlen($name, 'utf-8');
        $name_len = $name_len > 3 ? 3 : $name_len;
        for($j = 0; $j < $name_len; ++$j){
            $_name = mb_substr($name, $j, 1, 'utf-8');
            $_name = sc2pinyin($_name);
            $j < 2 && $_name = ucfirst($_name);
            $new_name .= $_name;
        }
        
        $name = $new_name . substr($mobile, -4);
    }else{
        $name = sc2pinyin($name, 'all', '', '/[a-zA-Z\d]/');
        $name = substr($name, 0, 12);
        $name .= substr($mobile, -4);
    }
    
    return $name;
}

if(!function_exists('sc2pinyin'))
{
    /**
     * 中文转拼音 (utf8版,gbk转utf8也可用) 此函数效率高
     ...
     * @return string             拼音字符串
     */
    function sc2pinyin($str, $ret_format = 'all', $placeholder = '', $allow_chars = '/[a-zA-Z\d ]/') 
	{
        static $pinyins = null;
	...
	* @param mixed $encode 字符编码
	* @return mixed 返回拼音字符串
	*/
	function _sc2pinyin($string, $encode='UTF-8')
	{
		...
		$data_value = "-20319|-20317|-20304|-20295|-20292|-20283|-20265|-20257|-20242|-20230|-20051|-20036|-20032|-20026|-20002|-19990".
		...
		$arr_data_value = explode('|', $data_value);
		$data = array_combine($arr_data_key, $arr_data_value);
		...
		if($encode!= 'GB2312'){
            ...
            $string = @iconv('UTF-8', 'GB2312//TRANSLIT', $string);
        }
